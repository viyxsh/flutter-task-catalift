...
class _HomeScreenState extends State<HomeScreen> {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            _buildSearchBar(),
            if (filteredFeaturedCourses.isNotEmpty)
              _buildFeaturedCoursesSlider(context, filteredFeaturedCourses)
            else
              const Padding(
                padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                child: Text(
                  'No featured courses match your search',
                  style: TextStyle(
                    fontSize: 16,
  ...
  Widget _buildFeaturedCoursesSlider(BuildContext context, List<Course> filteredFeaturedCourses) {
    return CarouselSlider.builder(
      itemCount: filteredFeaturedCourses.length,
      itemBuilder: (context, index, realIndex) {
        final course = filteredFeaturedCourses[index];
